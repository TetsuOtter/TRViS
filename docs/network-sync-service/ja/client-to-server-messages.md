...
| 要求メッセージ | `MessageType` を**持つ** | [§2](#2-requestserverinfo) / [§3](#3-requestdiagraminfo) / [§4](#4-searchtrain) / [§5](#5-requesttraintimetable) / [§6](#6-acknowledgenotification) |
...
[`AcknowledgeNotification`](#6-acknowledgenotification) は `RequestId` を
使わず、列車検索の機能ネゴシエーションとは無関係です。
