...
public sealed class V2RowTablet : ContentView
{
	...
	void ApplyCurrent()
	{
		...
		{
			// Normal cards sit on --card (white in light, elevated in dark) so they
			// lift off --bg; pass cards stay on --bg (≈ transparent-over-bg).
			_normalCardBorder.Background = _item.IsPass
				? ((Brush?)res?["OT_Bg"] ?? new SolidColorBrush(Colors.White))
				: ((Brush?)res?["OT_Card"] ?? new SolidColorBrush(Colors.White));
