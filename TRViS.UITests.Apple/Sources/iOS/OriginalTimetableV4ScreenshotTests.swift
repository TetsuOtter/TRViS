// OriginalTimetableV4ScreenshotTests.swift
// Visual regression baseline + diff for the V4 (Next Big) page.
// iPad mini A17 only — captures the rich tablet Hero + MiniList layout.

#if !targetEnvironment(macCatalyst)
import XCTest

class OriginalTimetableV4ScreenshotTests: BaseUITestCase {
    ...
    override func setUpWithError() throws {
        ...
        try XCTSkipIf(
            ...
            "Skipping V4 VRT: width \(width)pt < tablet breakpoint \(tabletBreakpointPt)pt."
        )

        try XCTSkipIf(
            ScreenshotBaselineHelper.baselineRoot.isEmpty,
            "Skipping V4 VRT: TRVIS_SCREENSHOT_BASELINE_DIR not set."
        )
    ...
    func testV4Tablet_Empty_Baseline() throws {
        let v4 = shell.navigateToOriginalTimetableV4()
        XCTAssertTrue(
            v4.waitForRendered(timeout: 30),
            "V4 page should render before capturing baseline."
        )
        XCTAssertTrue(
        ...
        settle()
        try captureOrCompare(screen: "originalTimetable-v4-tablet-empty")
    }

    func testV4Tablet_WithSelectedTrain_Baseline() throws {
        ...
        XCTAssertTrue(
            v4.isTrainStripeVisible(timeout: 15),
            "With-train baseline requires the TrainStripe header to be visible."
        )
        settle()
        Thread.sleep(forTimeInterval: 0.5)
        try captureOrCompare(screen: "originalTimetable-v4-tablet-with-train")
    }

    // MARK: — Capture helper (duplicated from V1; refactor opportunity later)
